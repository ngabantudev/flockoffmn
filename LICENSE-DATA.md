...
| Reported crime, small areas (block groups) | `crime-blockgroups.geojson` | [CC0 1.0](https://creativecommons.org/publicdomain/zero/1.0/) for the counts; public domain (U.S. federal work) for the boundaries | Free for any use. Aggregated by this project from the City's incident feed — the only layer here built that way. The ingest reads location and nothing else: no case number, address, date or charge is downloaded or stored, no offense breakdown is published at this scale, and areas under five reported offenses in a year are withheld. See the rules at the top of `scripts/ingest/mn/crime-blockgroups.mjs`. |
...
- **Demographics** — U.S. Census Bureau, American Community Survey, 5-Year
...
- **Reported crime, small areas** — City of Minneapolis Open Data, Police
  Incidents, aggregated to census block groups by this project; block group
  boundaries from U.S. Census Bureau TIGERweb.
