...
export const LAYERS: LayerDefinition[] = [
...

  {
    ...
    limitations: [
      ...
      {
        en: 'The cords are the shortest set of roads that joins every cluster to the rest, and nothing more than that. There is exactly one cord per join and no spare: where two clusters could have been joined by three different roads, one is drawn and the other two are not, so the absence of a cord between two places is never evidence that no road runs between them. Which road wins is decided by straight-line distance between the nearest readers of the two clusters, then routed; a few joins are missing entirely because no drivable route came back at all, and the data file counts those.',
        es: 'Los cordones son el conjunto más corto de vías que une cada grupo con el resto, y nada más. Hay exactamente un cordón por unión y ninguno de repuesto: donde dos grupos podrían haberse unido por tres vías distintas, se dibuja una y las otras dos no, así que la ausencia de un cordón entre dos lugares nunca prueba que no haya carretera entre ellos. Qué vía gana lo decide la distancia en línea recta entre los lectores más cercanos de ambos grupos, y luego se calcula la ruta; algunas uniones faltan del todo porque no se obtuvo ninguna ruta transitable, y el archivo de datos las cuenta.',
      },
      {
    ...
    detailFields: [
      ...
      },
      {
        key: 'bringsInSites',
        label: {
          en: 'Reader locations reaching the rest only through this cord',
          es: 'Ubicaciones de lectores que llegan al resto solo por este cordón',
        },
      },
      {
        key: 'operatorCount',
