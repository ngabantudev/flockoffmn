...
export const LAYERS: LayerDefinition[] = [
...

  {
    ...
    limitations: [
      ...
      {
        en: 'Connection is measured between the readers this layer holds, which is every camera that OpenStreetMap places on a named or numbered road. Cameras it does not — 143 of them, plus any nobody has mapped — are absent from the calculation entirely, so two networks shown as separate may in fact be bridged by readers that are not here to bridge them.',
        es: 'La conexión se mide entre los lectores que contiene esta capa, es decir, cada cámara que OpenStreetMap sitúa en una vía con nombre o número. Las que no están ahí —143 de ellas, más las que nadie ha mapeado— quedan fuera del cálculo por completo, así que dos redes que aparecen separadas podrían estar unidas por lectores que no figuran aquí.',
      },
      {
    ...
    linkRadius: {
      ...
      help: {
        en: 'How close two reader locations must be to belong to the same network. Widen it and separate pockets fuse, pulling in the side streets between them; narrow it and only the tightest clusters survive.',
        es: 'Qué tan cerca deben estar dos ubicaciones de lectores para pertenecer a la misma red. Amplíelo y los focos separados se fusionan, arrastrando consigo las calles intermedias; redúzcalo y solo sobreviven los conjuntos más compactos.',
      },
    },
